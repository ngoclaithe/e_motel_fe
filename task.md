# **Bổ sung Service `user.ts` và cập nhật form tạo hợp đồng**
...
- Với các endpoint cần xác thực, thêm header `Authorization: Bearer <access_token>`  
- Các hàm viết dạng async, trả về `data`

**Cấu trúc hàm đề xuất:**
...
## **Phần 3 — Cập nhật form tạo hợp đồng**
...
- Khi người dùng nhập số điện thoại, gọi:  
  ...
  thông qua `userService.searchByPhone(phone)`
...
2. Gọi API tìm người thuê  
3. Nếu tìm thấy → hiển thị thông tin, gán `tenantId`  
4. Nếu không thấy → báo lỗi và không cho gửi form  

---

### **Mẫu payload khi tạo hợp đồng**
